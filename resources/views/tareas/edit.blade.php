    ...
    <form action="{{ route('tareas.update', $tarea) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="shadow sm:rounded-md sm:overflow-hidden">
            <div class="px-4 py-5 bg-white sm:p-6">
                <div class="grid grid-cols-6 gap-6">
                    <div class="col-span-6">
                        ...
                        <input type="text" name="titulo" id="titulo" value="{{ old('titulo', $tarea->titulo) }}" required class="input-field">
                        @error('titulo')
                    ...
                    <div class="col-span-6">
                        ...
                        <textarea name="descripcion" id="descripcion" rows="3" class="input-field">{{ old('descripcion', $tarea->descripcion) }}</textarea>
                        @error('descripcion')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="col-span-6 sm:col-span-3">
                        ...
                        <select name="usuario_id" id="usuario_id" required class="input-field">
                            ...
                            @foreach($usuarios as $usuario)
                                <option value="{{ $usuario->id }}" {{ old('usuario_id', $tarea->usuario_id) == $usuario->id ? 'selected' : '' }}>
                    ...
                    <div class="col-span-6 sm:col-span-3">
                        ...
                        <input type="date" name="fecha_inicio" id="fecha_inicio" value="{{ old('fecha_inicio', $tarea->fecha_inicio) }}" class="input-field">
                        @error('fecha_inicio')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="col-span-6 sm:col-span-3">
                        ...
                        <input type="date" name="fecha_fin" id="fecha_fin" value="{{ old('fecha_fin', $tarea->fecha_fin) }}" class="input-field">
                        @error('fecha_fin')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="col-span-6">
                        ...
                        <select name="estado" id="estado" required class="input-field">
                            <option value="por hacer" {{ old('estado', $tarea->estado) == 'por hacer' ? 'selected' : '' }}>Por hacer</option>
            ...
            <div class="px-4 py-3 bg-gray-50 text-right sm:px-6">
                <a href="{{ route('tareas.show', $tarea) }}" class="btn btn-gray mr-2">
                    Cancelar
                </a>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        proyectoSelect.addEventListener('change', function() {
            ...
            if (proyectoId) {
                ...
                window.location.href = "{{ route('tareas.edit', $tarea) }}?proyecto_id=" + proyectoId;
            }
        });
    });
</script>
